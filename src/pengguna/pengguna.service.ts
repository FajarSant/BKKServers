...
async importFromExcel(file: Express.Multer.File) {
  ...
  for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
    ...
    if (!email || !nama || !peranStr || !jenisKelaminStr || !nisn) {
      invalidRows.push(`Baris ${rowNumber}: Email, Nama, Peran, NISN, atau Jenis Kelamin tidak lengkap.`);
      continue;
    }
    ...
    if (!validJenisKelamin.includes(jenisKelaminStr)) {
      invalidRows.push(`Baris ${rowNumber}: Jenis Kelamin tidak valid. Harus laki_laki atau perempuan.`);
      continue;
    }
    ...
    if (existing) {
      invalidRows.push(`Baris ${rowNumber}: Email atau NISN sudah ada, dilewati.`);
      continue;
    }
    ...
    importedData.push({
      ...
      telepon,
      tanggalLahir,
      jenisKelamin: jenisKelaminStr as Prisma.PenggunaCreateManyInput['jenisKelamin'],
      katasandi: hashedPassword,
    });
  }
